...
def _make_synthetic_dataloaders(batch_size: int, device):
    """
    Synthetic multi-class dataset (10 classes, 2 000 samples, 128 features).
...
def make_dataloaders(batch_size: int = 256, data_root: str = "./data"):
    """
    Tries to load MNIST via torchvision; falls back to synthetic dataset.
    Returns (train_loader, val_loader, input_dim, num_classes).
    """
    try:
        import torchvision
...
class MLP(nn.Module):
    ...
    Final layer: Linear (logits, no activation – used with CrossEntropyLoss).
    """
...
def macro_f1(all_preds: torch.Tensor, all_labels: torch.Tensor,
    ...
    """
    Compute macro-averaged F1 score from flat prediction / label tensors.
    Pure-PyTorch implementation; no sklearn required.
    """
    f1s = []
...
def train(model, train_loader, val_loader, device,
    ...
    """
    Full training loop using Adam + CrossEntropyLoss + autograd.
